#!/usr/bin/env python3
# 2024.10.23 Start Worker for Qwen2.5-14B
import sys
...
def main():
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python step3_start_worker_qwen14b.py <scheduling_method> [worker_ip]")
    ...
    print(f"Starting Qwen2.5-14B worker with scheduling method: {scheduling_method}.")
    if worker_ip:
        print(f"Using specified worker IP: {worker_ip}")

    # run worker with Qwen2.5-14B model
