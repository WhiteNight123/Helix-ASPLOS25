#!/usr/bin/env python3
# 2024.10.27 Start Worker for Qwen3-32B
import sys
...
def main():
    ...
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python step3_start_worker_qwen32b.py <scheduling_method> [worker_ip]")
    ...
    print(f"Starting Qwen3-32B worker with scheduling method: {scheduling_method}.")
    if worker_ip:
        print(f"Using specified worker IP: {worker_ip}")

    # run worker with Qwen3-32B model
