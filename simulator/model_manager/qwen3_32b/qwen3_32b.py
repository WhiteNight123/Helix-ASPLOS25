# 2024.10.27 Qwen3-32B Statistics
...
class Qwen32BStatistics(ModelStatistics):
    def __init__(self, num_machines_dict: Dict[str, int]):
        ...
        if "RTX2080Ti" in num_machines_dict:
            self.rtx2080ti = Qwen32BonRTX2080Ti(
                num_machines_dict=num_machines_dict,
                typical_layers_dict=typical_layers_dict,
                normalized_perf_dict=normalized_perf_dict
            )
        ...
        self.activation_size: float = 5120 * 2 * Byte  # hidden_size * 2 bytes (same as Qwen2.5-14B)
        # Qwen3-32B model with 64 layers
        # Each layer is approximately 0.5GB (32B params / 64 layers = 0.5B params per layer)
        self.model_param_sizes = [0.5 * GB] * 64
        assert len(self.model_param_sizes) == Qwen3_32B_TOTAL_LAYERS, "Total layer number mismatch!"
