# 2024.10.23 Qwen2.5-14B Statistics
...
from simulator.model_manager.qwen2_5_14b.rtx2080ti.qwen2_5_14b_rtx2080ti import Qwen14BonRTX2080Ti
from simulator.event_simulator.utils import Byte, GB, Qwen2_5_14B_TOTAL_LAYERS


class Qwen14BStatistics(ModelStatistics):
    def __init__(self, num_machines_dict: Dict[str, int]):
        """
        This class stores the profiling results of different machines running Qwen2.5-14B.
        ...
        typical_layers_dict = {"RTX2080Ti": 12}
        total_layer_capacity = 0
        for machine_name in num_machines_dict:
            total_layer_capacity += num_machines_dict[machine_name] * typical_layers_dict.get(machine_name, 12)
        
        # Adjust if capacity is low
        if total_layer_capacity < Qwen2_5_14B_TOTAL_LAYERS * 1.2:
            typical_layers_dict = {"RTX2080Ti": 12}  # Maximum layers per GPU
        typical_layers_dict = {m_type: typical_layers_dict.get(m_type, 12) for m_type in num_machines_dict}

        # estimate the normalized performance
        # RTX2080Ti is comparable to T4 in inference, slightly better in some cases
        normalized_perf_dict = {"RTX2080Ti": 16}  
        ...
        if "RTX2080Ti" in num_machines_dict:
            self.rtx2080ti = Qwen14BonRTX2080Ti(
                num_machines_dict=num_machines_dict,
                typical_layers_dict=typical_layers_dict,
                normalized_perf_dict=normalized_perf_dict
            )
        ...
        # Qwen2.5-14B model with 48 layers
        # Each layer is approximately 0.3GB (14B params / 48 layers ≈ 0.29B params per layer)
        self.model_param_sizes = [0.3 * GB] * 48
        assert len(self.model_param_sizes) == Qwen2_5_14B_TOTAL_LAYERS, "Total layer number mismatch!"
