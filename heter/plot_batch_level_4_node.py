...
# 解析4个服务器的日志文件
print("解析server1日志...")
server1_events, server1_requests = parse_log_file('/root/Helix-ASPLOS25/examples/real_sys/log/timeline_Worker_L0-10.log', 'server1')
print("解析server2日志...")
server2_events, server2_requests = parse_log_file('/root/Helix-ASPLOS25/examples/real_sys/log/timeline_Worker_L10-20.log', 'server2')
print("解析server3日志...")
server3_events, server3_requests = parse_log_file('/root/Helix-ASPLOS25/examples/real_sys/log/timeline_Worker_L20-30.log', 'server3')
print("解析server4日志...")
server4_events, server4_requests = parse_log_file('/root/Helix-ASPLOS25/examples/real_sys/log/timeline_Worker_L30-40.log', 'server4')
...
for i, (events, name) in enumerate(zip(servers_events, server_names)):
    ...
    for batch_id, batch_events in events.items():
        print(f"  Batch {batch_id}: {len(batch_events)} 个事件")
...
# 设置要显示的时间范围（根据实际数据调整）
start_time = 1762150456.6301918
end_time = 1762150456.9301918
...
def draw_compute_segments(events, resource_level, server_name):
    for batch_id, batch_events in events.items():
        ...
        if len(compute_starts) != len(compute_ends):
            print(f"警告: {server_name} Batch {batch_id} 的计算开始({len(compute_starts)})和结束({len(compute_ends)})事件数量不匹配，取最小值 {min_count} 进行匹配")
        ...
        if s1_trans_starts and s2_recvs:
            for i in range(min(len(s1_trans_starts), len(s2_recvs))):
                ...
                if norm_end_ts > norm_start and norm_start_ts < norm_end:
                    ...
                    annotations.append(ann)
                    found_transfers.append(("S1->S2", batch_id, start, end))
    
    # S2 -> S3 传输
    print("绘制 S2->S3 传输...")
    for batch_id in set(list(server2_events.keys()) + list(server3_events.keys())):
        s2_trans_starts = [ts for etype, ts, _, _ in server2_events.get(batch_id, []) 
                          if etype == 'trans_start']
        s2_trans_starts.sort()
        
        s3_recvs = [ts for etype, ts, _, _ in server3_events.get(batch_id, []) 
                   if etype == 'recv']
        s3_recvs.sort()
        ...
        if s2_trans_starts and s3_recvs:
            for i in range(min(len(s2_trans_starts), len(s3_recvs))):
                ...
                if norm_end_ts > norm_start and norm_start_ts < norm_end:
                    ...
                    annotations.append(ann)
                    found_transfers.append(("S2->S3", batch_id, start, end))
    
    # S3 -> S4 传输
    print("绘制 S3->S4 传输...")
    for batch_id in set(list(server3_events.keys()) + list(server4_events.keys())):
        s3_trans_starts = [ts for etype, ts, _, _ in server3_events.get(batch_id, []) 
                          if etype == 'trans_start']
        s3_trans_starts.sort()
        
        s4_recvs = [ts for etype, ts, _, _ in server4_events.get(batch_id, []) 
                   if etype == 'recv']
        s4_recvs.sort()
        ...
        if s3_trans_starts and s4_recvs:
            for i in range(min(len(s3_trans_starts), len(s4_recvs))):
                ...
                if norm_end_ts > norm_start and norm_start_ts < norm_end:
                    ...
                    annotations.append(ann)
                    found_transfers.append(("S3->S4", batch_id, start, end))
    
    # 返回传输 (所有服务器返回结果到S1)
    print("绘制返回传输...")
    # S4 -> S3 返回
    for batch_id in set(list(server4_events.keys()) + list(server3_events.keys())):
        s4_trans_starts = [ts for etype, ts, _, _ in server4_events.get(batch_id, []) 
                          if etype == 'trans_start']
        s4_trans_starts.sort()
        
        s3_backs = [ts for etype, ts, _, _ in server3_events.get(batch_id, []) 
        ...
        if s4_trans_starts and s3_backs:
            for i in range(min(len(s4_trans_starts), len(s3_backs))):
                ...
                if norm_end_ts > norm_start and norm_start_ts < norm_end:
                    ...
                    annotations.append(ann)
                    found_transfers.append(("S4->S3", batch_id, start, end))
    
    # S3 -> S2 返回
    for batch_id in set(list(server3_events.keys()) + list(server2_events.keys())):
        s3_trans_starts = [ts for etype, ts, _, _ in server3_events.get(batch_id, []) 
                          if etype == 'trans_start']
        s3_trans_starts.sort()
        
        s2_backs = [ts for etype, ts, _, _ in server2_events.get(batch_id, []) 
                   if etype == 'back_to_head']
        s2_backs.sort()
        ...
        if s3_trans_starts and s2_backs:
            for i in range(min(len(s3_trans_starts), len(s2_backs))):
                ...
                if norm_end_ts > norm_start and norm_start_ts < norm_end:
                    ...
                    annotations.append(ann)
                    found_transfers.append(("S3->S2", batch_id, start, end))
    
    # S2 -> S1 返回
    for batch_id in set(list(server4_events.keys()) + list(server1_events.keys())):
        s4_trans_starts = [ts for etype, ts, _, _ in server4_events.get(batch_id, []) 
                          if etype == 'trans_start']
        s4_trans_starts.sort()
        ...
        if s4_trans_starts and s1_backs:
            for i in range(min(len(s4_trans_starts), len(s1_backs))):
                ...
                if norm_end_ts > norm_start and norm_start_ts < norm_end:
                    ...
                    annotations.append(ann)
                    found_transfers.append(("S4->S1", batch_id, start, end))
